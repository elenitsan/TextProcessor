...
import datamodel.buildingblocks.Document.DocumentTypeEnum;
...
public class Engine implements IPlainTextDocumentEngine {
    ...
    public Engine(String pFilePath, String pInputType, String pAlias) {
        DocumentTypeEnum docType = DocumentTypeEnum.RAW;
        if (pInputType.equalsIgnoreCase("ANNOTATED"))
            docType = DocumentTypeEnum.ANNOTATED;
...
//			prefixes.add(l.get(2));
    ...
    public int exportMarkDown(String outputFileName) {
        ...
        if (this.document.getInputFileType().equals(DocumentTypeEnum.ANNOTATED)) {
            for (LineBlock lineBlock : document.getLineBlocks()) {
                String firstRow = lineBlock.getLines().get(0);
                String prefix = prefixes.stream().filter(firstRow::startsWith).findFirst().orElse(null);
                if (prefix != null) {
                    lineBlock.getLines().set(0, firstRow.replaceFirst(prefix, ""));
                }
            }
        }
    ...
    private void characterizeLineblocks(Document document, RuleSet ruleSet) {
        ...
        if (document.getInputFileType() == DocumentTypeEnum.RAW)
            characterizeRawFile(lineblocks, ruleSet);
        else if (document.getInputFileType() == DocumentTypeEnum.ANNOTATED)
